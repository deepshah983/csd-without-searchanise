...
import CsdShippingTimer from '../csd/shipping-timer';
import csdProductWishlistCartCount from '../csd/product-wishlist-cart-count';
import csdSwymWishlistPlus from '../csd/swym-wishlist-plus';
import { setInterval, setTimeout } from 'core-js';
import CsdGTM from '../csd/google-tag-manager';
...
export default class ProductDetails extends ProductDetailsBase {
    constructor($scope, context, productAttributesData = {}) {
        ...
        CsdShippingTimer.load(context, $scope);
        csdProductWishlistCartCount.load(context, $scope);
        csdSwymWishlistPlus.handleProductDetails($scope);
    }
    ...
    showProductImage(image) {
        if (isPlainObject(image)) {
            ...
            // mobile slider
            if($(".small-slider .slick-active a").hasClass("variant-link")){
                $(".small-slider .slick-active a.variant-link").show();
                $(".small-slider .slick-active a.variant-link").attr('href', mainImageUrl);
                $(".small-slider .slick-active a.variant-link img").attr('srcset', mainImageSrcset);
                $(".small-slider .slick-active a.variant-link img").attr('src', mainImageUrl);
            }else{
                $(".small-slider .slick-active a.main-link").hide();
                $('<a data-fancybox class="productView-thumbnail-link variant-link" href=" '+mainImageUrl+'"><img src=" '+mainImageUrl+' " srcset=" '+mainImageSrcset+' " alt=" '+image.alt+' " title=" '+image.alt+' " class="productView-image--default lazyloaded"></a>').insertAfter(".small-slider .slick-active a");
            }
            // desktop slider
            var data_zoom_image = $(".slider_image .slick-active figure").attr('data-zoom-image');
            if($(".slider_image .slick-active a").hasClass("variant-link")){
                $(".slider_image .slick-active a").attr('href', mainImageUrl);
                $(".slider_image .slick-active img").attr('srcset', mainImageSrcset);
                $(".slider_image .slick-active img").attr('src', mainImageUrl);
            }else{
                $(".slider_image .slick-active a.variant-link").hide();
                $('<a class="variant-link" href=" '+mainImageUrl+'"><img src=" '+mainImageUrl+' " srcset=" '+mainImageSrcset+' " alt=" '+image.alt+' " title=" '+image.alt+' " class="productView-image--default lazyloaded"></a>').insertAfter(".slider_image .slick-active a");
            }
            
            setInterval(function(){
                var data_zoom_image = $(".slider_image .slick-active figure").attr('data-zoom-image');
                if($(".slider_image .slick-active .easyzoom-flyout").hasClass("image-loaded")){

                }else{
                    $(".slider_image .slick-active .easyzoom-flyout").addClass("image-loaded");
                    $(".slider_image .slick-active .easyzoom-flyout img").attr('src', data_zoom_image);
                    $(".slider_image .slick-active .easyzoom-flyout img").attr('srcset', data_zoom_image);
                    // console.log(data_zoom_image);
                }
            }, 500);
        } else {
    ...
    addProductToCart(event, form) {
        ...
        utils.api.cart.itemAdd(normalizeFormData(new FormData(form)), (err, response) => {
            ...
            // CSD EDIT
            try {
                const normalizedForm = normalizeFormData(new FormData(form))
                csdProductWishlistCartCount.trackAddProductToCart( normalizedForm.get('product_id') );
            } catch (err) {
